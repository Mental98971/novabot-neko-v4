...
# Clean unsupported query parameters for asyncpg (e.g., sslmode, channel_binding from Neon)
_db_url = make_url(settings.database_url)
if not _is_sqlite:
    _query_params = dict(_db_url.query)
    _sslmode = _query_params.pop("sslmode", None)
    _query_params.pop("channel_binding", None)
    _db_url = _db_url.set(query=_query_params)

_engine_kwargs: dict = {"echo": settings.debug, "pool_pre_ping": not _is_sqlite}
if not _is_sqlite:
    _engine_kwargs.update(pool_size=20, max_overflow=10)
    if _sslmode or "ssl" in settings.database_url.lower():
